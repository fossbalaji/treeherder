...
import { JobGroupComponent } from './groups';
  ...
  render() {
    return (
      <tr id={this.props.platform.id}
        ...
        <JobPlatformDataComponent $injector={this.props.$injector}
                                  platform={this.props.platform} />
        {/*<job-data-component $injector={this.props.$injector}*/}
...
class JobTableComponent extends React.Component {
  constructor(props) {
    ...
    const thPlatformName = this.props.$injector.get('thPlatformName');

    // Check for a selected job in the result set store
    let selectedJobId = null;
    let selectedJobObj = this.ThResultSetStore.getSelectedJob(this.$rootScope.repoName);
      ...
      this.state.jobsLoaded ?
        <table id={this.aggregateId} className="table-hover">
          ...
          {Object.keys(this.state.platforms).map((id, i) => (
            // this.state.platforms[id].visible &&
              <JobTableRowComponent $injector={ this.props.$injector }
...
class ResultSetComponent extends React.Component {
  constructor(props) {
    super(props);
    const $injector = this.props.$injector;
  ...
  render() {
    ...
    return (
      <div className="row result-set clearfix">
        {this.state.showRevisions ? <RevisionList resultset={this.props.resultset}
                                                repo={this.$rootScope.currentRepo}
                                                $injector={this.props.$injector} />
                                                : <span></span> }
        <span className={ containerClasses.join(' ') }>
          <JobTableComponent resultset={this.props.resultset}
                             $injector={this.props.$injector} />
        </span>
      </div>
    );
...
treeherder.directive('resultSet', ['reactDirective', '$injector', (reactDirective, $injector) =>
